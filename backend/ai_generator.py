...
"""
    ...
    def _execute_tool_rounds(
        ...
        logger.debug(
            f"Starting sequential tool calling, max rounds: {config.MAX_TOOL_ROUNDS}"
        )
        ...
        for round_num in range(config.MAX_TOOL_ROUNDS):
            ...
            if response.stop_reason != "tool_use":
                logger.debug(
                    f"No tool use in round {round_num + 1}, stop_reason: {response.stop_reason}"
                )
            ...
            if tool_results:
                logger.debug(
                    f"Adding {len(tool_results)} tool results to message history"
                )
            ...
            if msg["role"] == "user" and isinstance(msg["content"], list):
                for content_item in msg["content"]:
                    ...
                    if (
                        ...
                        if (
                            tool_content and len(str(tool_content)) > 50
                        ):  # Only include substantial results
...
Please provide a direct answer to the question."""
            ...
            final_message = (
                f"""Please answer this question about course materials: {query}"""
            )
