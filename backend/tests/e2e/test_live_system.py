...
class TestLiveSystem:
    ...
    def test_real_query_with_api_key_check(self):
        ...
        if not api_key or api_key == "" or api_key.startswith("your_key"):
            pytest.skip(
                "No valid ANTHROPIC_API_KEY found - this would cause query failures"
            )
        ...
        try:
            ...
            if response.lower().strip() == "query failed":
                pytest.fail(
                    "Response is exactly 'query failed' - this is the bug we're looking for!"
                )
    ...
    def test_tool_execution_in_live_system(self):
        ...
        try:
            ...
            response, sources = rag_system.query(
                "Tell me about MCP in the course materials"
            )
            ...
            if "MCP" not in response and len(sources) == 0:
                pytest.fail(
                    "Query about MCP didn't return MCP-related content or sources"
                )
    ...
    def test_error_handling_with_invalid_query(self):
        ...
        try:
            ...
            assert (
                response.lower().strip() != "query failed"
            ), "Empty query returned 'query failed'"
