...
class TestErrorPropagationDebug:
    ...
    def test_anthropic_api_connectivity(self):
        ...
        try:
            ...
            assert "API working" in response.content[0].text or "working" in response.content[0].text.lower()
    ...
    def test_vector_store_error_handling(self):
        ...
        try:
            invalid_vector_store = VectorStore("/invalid/path", config.EMBEDDING_MODEL, config.MAX_RESULTS)
        ...
        try:
            ...
            results = vector_store.search("nonexistent content", course_name="nonexistent course")
            print(f"Search on nonexistent content: Error={results.error}, Documents={len(results.documents)}")
            
            # Test course name resolution that should fail gracefully
            resolved = vector_store._resolve_course_name("completely_nonexistent_course_name")
    ...
    def test_ai_generator_error_scenarios(self):
        ...
        try:
            ...
            invalid_tool_manager.get_tool_definitions.return_value = [{"invalid": "tool_def"}]
    ...
    def test_anthropic_specific_errors(self):
        ...
        try:
            ...
            if "tool" in str(e).lower():
                print("  This is likely a tool-related error that could cause Query failed")
