...
class TestMCPCourseResolution:
        ...
        for variation in variations:
            ...
            try:
                ...
                if resolved:
                    ...
                    if matching_course:
                        print(
                            f"  Course metadata found: {matching_course.get('title')}"
                        )
    ...
    def test_lesson_5_content_search(self, vector_store):
        ...
        search_queries = ["lesson 5", "レッスン5", "fifth lesson", "lesson five", "5"]

        for query in search_queries:
            print(f"\nTesting search query: '{query}'")
            try:
                results = vector_store.search(
                    query=query, course_name=course_name, lesson_number=5
                )
    ...
    def test_lesson_content_by_number(self, vector_store):
        ...
        for lesson_num in range(1, 11):
            ...
            try:
                results = vector_store.search(
                    query="content", course_name=course_name, lesson_number=lesson_num
                )
        ...
        try:
            ...
            if "No relevant content found" not in mcp_lesson5_result:
                ...
                similar_topics_result = tool_manager.execute_tool(
                    "search_course_content", query="similar topics across courses"
                )
