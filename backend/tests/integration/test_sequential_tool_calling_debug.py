...
class TestSequentialToolCallingDebug:
    ...
    def test_step_by_step_tool_execution(self, tool_manager):
        ...
        try:
            overview_result = tool_manager.execute_tool(
                "get_course_overview", course_title="MCP"
            )
            print(f"Overview result: {overview_result[:200]}...")
            assert (
                "error" not in overview_result.lower()
                or "not found" not in overview_result.lower()
            )
        except Exception as e:
        ...
        try:
            topic_search = tool_manager.execute_tool(
                "search_course_content", query="similar topics"
            )
        ...
        for variation in course_variations:
            ...
            try:
                result = tool_manager.execute_tool(
                    "get_course_overview", course_title=variation
                )
                print(
                    f"Result for '{variation}': {'SUCCESS' if 'Course Title:' in result else 'FAILED'}"
                )
    ...
    def test_sequential_api_calls_simulation(self, ai_generator):
        ...
        try:
            ...
            print(
                f"Tool call arguments: {mock_tool_manager.execute_tool.call_args_list}"
            )
    ...
    def test_error_handling_in_sequential_calls(self, ai_generator):
        ...
        try:
            ...
            print(
                f"Error handling FAILED - exception not caught: {type(e).__name__}: {str(e)}"
            )
            traceback.print_exc()
            raise
    ...
    def test_timeout_scenarios(self, ai_generator):
        ...
        try:
            ...
            print(
                f"Timeout test completed in {end_time - start_time:.2f}s: {response[:200]}..."
            )
