...
class TestVectorSearchIntegration:
    ...
    def test_vector_store_has_data(self, real_vector_store):
        ...
        assert (
            course_count > 0
        ), f"No courses found in vector store. Expected > 0, got {course_count}"
        assert (
            len(existing_titles) > 0
        ), f"No course titles found. Expected > 0, got {len(existing_titles)}"
    ...
    def test_vector_store_basic_search(self, real_vector_store):
        ...
        test_queries = ["Python", "programming", "introduction", "course", "lesson"]
        ...
        for query in test_queries:
            ...
            print(
                f"[DEBUG] Sample metadata: {results.metadata[:2] if results.metadata else 'None'}"
            )

            # Check for search errors
            assert (
                results.error is None
            ), f"Vector store search failed for query '{query}': {results.error}"
            ...
            pytest.fail(
                "No search queries returned results - vector store may be empty or misconfigured"
            )
        ...
        for query in test_queries:
            ...
            try:
                ...
                if "query failed" in result.lower():
                    pytest.fail(
                        f"CourseSearchTool.execute returned 'query failed' for query '{query}': {result}"
                    )

                if result.strip() == "":
                    pytest.fail(
                        f"CourseSearchTool.execute returned empty string for query '{query}'"
                    )
                ...
                if "No relevant content found" not in result and query.strip():
                    ...
                    print(
                        f"[DEBUG] Sources tracked: {len(search_tool_real.last_sources)}"
                    )
                    print(f"[DEBUG] Source details: {search_tool_real.last_sources}")

            except Exception as e:
                pytest.fail(
                    f"CourseSearchTool.execute raised exception for query '{query}': {str(e)}"
                )
    ...
    def test_course_search_tool_with_filters(self, search_tool_real, real_vector_store):
        ...
        result = search_tool_real.execute(
            "introduction", course_name="NonexistentCourse123"
        )
        print(f"[DEBUG] Invalid course filter result: {result}")

        # Should return appropriate "not found" message, not "query failed"
        assert (
            "query failed" not in result.lower()
        ), f"Invalid course filter caused 'query failed': {result}"
    ...
    def test_chroma_database_integrity(self, real_vector_store):
        ...
        try:
            ...
            course_count = real_vector_store.get_course_count()
            print(
                f"[DEBUG] Successfully accessed database, course count: {course_count}"
            )
        except Exception as e:
