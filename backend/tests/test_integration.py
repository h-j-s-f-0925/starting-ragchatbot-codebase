...
class TestRAGSystemIntegration:
    ...
    def test_api_key_configuration(self):
        ...
        assert api_key != "", "ANTHROPIC_API_KEY is empty - this will cause 'query failed'"
        assert api_key is not None, "ANTHROPIC_API_KEY is None"
        assert not api_key.startswith("your_key_here"), "ANTHROPIC_API_KEY is not set to a real key"
    ...
    def test_vector_store_has_data(self):
        ...
        if course_count == 0:
            pytest.fail("Vector store has no courses loaded - this will cause 'No relevant content found'")
    ...
    def test_vector_store_search_functionality(self):
        ...
        print(f"Search results documents: {results.documents[:2] if results.documents else 'None'}")
        
        if results.error:
            pytest.fail(f"Vector store search failed with error: {results.error}")
        
        if results.is_empty():
            pytest.fail("Vector store search returned no results - check if data is indexed properly")
    ...
    def test_simulated_query_without_anthropic(self):
        ...
        with patch('rag_system.AIGenerator') as mock_ai_gen_class:
            mock_ai_gen = Mock()
            mock_ai_gen.generate_response.return_value = "Mocked AI response about Python"
            ...
            print(f"AI generator called with tools: {call_args.get('tools') is not None}")
            print(f"AI generator called with tool_manager: {call_args.get('tool_manager') is not None}")
