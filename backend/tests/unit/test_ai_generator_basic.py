...
class TestAIGeneratorBasic:
    ...
    def test_conversation_history_system_prompt_format(self, ai_generator, mock_anthropic_client):
        ...
        assert "You are an AI assistant" in system_content  # Base system prompt should be included
        assert "Previous conversation:" in system_content
        assert history in system_content
        assert system_content.endswith(history)  # History should be at the end

    def test_no_conversation_history_system_prompt(self, ai_generator, mock_anthropic_client):
    ...
    def test_multiple_content_blocks_response(self, ai_generator, mock_anthropic_client):
    ...
    @pytest.mark.parametrize("stop_reason,expected_behavior", [
        ("end_turn", "normal_completion"),
        ("max_tokens", "normal_completion"),
        ("stop_sequence", "normal_completion"),
    ])
    def test_different_stop_reasons(self, ai_generator, mock_anthropic_client, stop_reason, expected_behavior):
    ...
    def test_backward_compatibility_no_tools_parameter(self, ai_generator, mock_anthropic_client):
