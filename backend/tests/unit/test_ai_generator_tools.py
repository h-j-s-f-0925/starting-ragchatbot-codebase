...
class TestAIGeneratorTools:
    ...
    def test_tool_execution_error_handling(
        ...
        result = ai_generator.generate_response(
            "Search for something", tools=sample_tools, tool_manager=mock_tool_manager
        )
    ...
    def test_handle_tool_execution_no_tool_manager(
        ...
        result = ai_generator.generate_response(
            "Search for something", tools=sample_tools, tool_manager=None
        )
    ...
    def test_tool_execution_exception_handling(
        ...
        final_response.content = [Mock()]
        final_response.content[0].text = (
            "I encountered an error but here's what I can tell you."
        )
    ...
    def test_partial_tool_failure(
        ...
        final_response.content = [Mock()]
        final_response.content[0].text = (
            "Based on partial results, here's the information."
        )
    ...
    def test_tool_content_and_text_mixed_response(
        self, ai_generator, mock_anthropic_client, mock_tool_manager, sample_tools
    ):
    ...
    def test_backward_compatibility_tools_without_manager(
        self, ai_generator, mock_anthropic_client, sample_tools
    ):
        ...
        result = ai_generator.generate_response(
            "Test query", tools=sample_tools, tool_manager=None
        )
    ...
    def test_backward_compatibility_no_tools(
        ...
        result = ai_generator.generate_response(
            "Test query", tools=None, tool_manager=mock_tool_manager
        )
